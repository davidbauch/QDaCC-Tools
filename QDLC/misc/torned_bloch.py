# Modified Bloch Function from QuTip
...
class Bloch:
    ...
    def __init__(self, fig=None, axes=None, view=None, figsize=None,
        ...
        self.trajectory = [[[],[],[]]]
        self.ls = LightSource(azdeg=0, altdeg=65)

        self.colormap_sphere = cm.Greys_r
        self.colormap_data = 'coolwarm'
    ...
    def add_points(self, points, meth='s', alpha=1.0, current = None):
    ...
    def add_annotation(self, state_or_vector, text, **kwargs):
        ...
        Parameters
        ----------
        state_or_vector : array/list/tuple
        ...
        """
        if isinstance(state_or_vector, (list, ndarray, tuple)) \
    ...
    def render(self, trail_points = None):
        """
    ...
    def plot_back(self):
        ...
        if isinstance(self.colormap_sphere,str):
            self.colormap_sphere = cm.get_cmap(self.colormap_sphere)
        rgb = self.ls.shade(z, vert_exag=0.1, blend_mode='soft', cmap=self.colormap_sphere)
        self.axes.plot_surface(x, y, z, alpha=self.sphere_alpha, linewidth=0, shade=False, facecolors=rgb)
        # wireframe
    ...
    def plot_points(self, trail_points: int | None, plot_trajectory: bool = True):
        if isinstance(self.colormap_data, str):
            self.colormap_data = plt.get_cmap(self.colormap_data)
        for k in range(len(self.trajectory)):
            color = self.colormap_data(k/len(self.trajectory))
            if trail_points is None:
                ...
                if plot_trajectory:
                    self.axes.plot(self.trajectory[k][1],self.trajectory[k][0],self.trajectory[k][2],alpha=0.25, zdir='z',linewidth=self.point_size,color=color)
                x = self.trajectory[k][1][-trail_points:]
                y = self.trajectory[k][0][-trail_points:]
                z = self.trajectory[k][2][-trail_points:]
                ...
                # Create a colormap that goes from opaque to transparent
                colors = self.colormap_data(k/(n+1)*np.ones(n))
                #colors = cmap(np.linspace(0, 1, trail_points))
                colors[:, -1] = np.linspace(0, 1, n)

                # Create a LineCollection
                lc = Line3DCollection(segments, colors=colors, linewidth=self.trail_point_size)
